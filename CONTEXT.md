...
- **BoxDecorationPlan** — backend-agnostic box-decoration geometry in `takumi-core/src/layout/decoration.rs`, resolved once and consumed by both backends (no compositing inside). Landed: `ClipBox` (border/padding/content clip regions per `background-clip`) and `OutlineGeometry` (the outline ring). Raster translates them to tiny-skia ops; SVG to vector paths. Still duplicated (next slice): box-shadow spread geometry + mask offsets — `outset_shadow_box` is already a shared `BorderProperties` method, but the orchestration around it is mirrored. Removes the drift the `node_paint.rs` "candidate for promotion" note predicted.
