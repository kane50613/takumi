...
fn test_tw_loses_to_a_stylesheet_rule() {
  ...
  let card = |tw: &str| {
    Node::container([])
      .with_class_name("card".to_owned())
      .with_tw(TailwindValues::from_str(tw).expect("tailwind values should parse"))
  };

  let node = Node::container([
    // The stylesheet rule wins: it is unlayered, the utility is not.
    card("bg-red-500 w-full h-32"),
    // `!` puts the utility in the important half, which reverses layer order.
    card("bg-red-500! w-full h-32"),
  ])
  .with_tw(
