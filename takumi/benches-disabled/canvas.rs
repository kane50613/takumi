...
fn bench_canvas(c: &mut Criterion) {
  ...
  group.bench_function("nested_clip_masks", |b| {
    b.iter(|| render_node(&font_context, black_box(nested_clip_masks_fixture())))
  });
  group.bench_function("scaled_image_clip", |b| {
    b.iter(|| render_node(&font_context, black_box(scaled_image_fixture())))
  });
  group.bench_function("gradient_clip_mask", |b| {
    b.iter(|| render_node(&font_context, black_box(gradient_clip_mask_fixture())))
  });
