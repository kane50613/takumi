  ...
  hooks: {
    ...
    "build:done": () => {
      for (const name of exportOutputs) {
        ...
        if (patched.includes('new URL("takumi_paint_wasm_bg.wasm"')) {
...
function patchGeneratedExportScript(content: string) {
  return content
    .replace(
      'if (module_or_path === void 0) module_or_path = new URL("takumi_paint_wasm_bg.wasm", import.meta.url);\n',
      "",
    )
    .replace(
      'if (module_or_path === void 0) module_or_path = new URL("takumi_paint_wasm_bg.wasm", require("url").pathToFileURL(__filename).href);\n',
      "",
    );
}
