...
Nodes come in the order the renderer paints them, so `zIndex` does not appear. Iterating the tree yields them in that order. Within a node, draw `shadows.outer`, `background`, `shadows.inset`, and `border`, then `image`, `inlineBackgrounds`, and `textRuns`, then `children`, then `outline`.
