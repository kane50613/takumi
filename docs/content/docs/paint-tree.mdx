...
Each run carries its `font`: the registered family, the face index, the weight and style after variations, and any synthetic bold or skew the shaper applied. `tree.textRuns()` yields every run with the node that paints it, and `tree.find(id)` looks a node up by its source `id`.
