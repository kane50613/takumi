...
Takumi renders `display: table` by rewriting the table into a grid before layout, so every row shares one set of column tracks. It is a naive approximation, not the CSS table algorithm. This page lists what carries over and where it drifts.
