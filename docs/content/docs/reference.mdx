...
| `contain`                                                            | `none`, `content`, `layout`, `style`, `paint` <sup>2</sup>                                                                                                                                                                                         |
| `content`                                                            | Supported                                                                                                                                                                                                                                          |
...
<sup>2</sup> `size`, `inline-size` and `strict` are rejected, because takumi does not implement size
containment. `style` containment parses but has nothing to scope: takumi has no author-facing
counters, and list-item ordinals do not restart at the boundary.

`contain` is not a performance hint here. takumi renders once, so there is no relayout for
containment to skip, and `contain: paint` costs a stacking context and a clip layer per box. Reach
for `overflow: clip` and `display: flow-root` when you want the effect without the containment.
...
| `flexFlow`                                               | Supported                                                          |
| `flexWrap`                                               | `nowrap`, `wrap`, `wrap-reverse`, each optionally with `balance`   |
| `flexLineCount`                                          | `<integer>` of 1 or more                                           |
| `justifyContent`, `alignContent`                         | Supported                                                          |
| `justifyItems`, `justifySelf`, `alignItems`, `alignSelf` | Supported, including `self-start` and `self-end`                   |
