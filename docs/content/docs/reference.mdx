...
| `boxShadow`                                 | Supported                                                                                                                     |
| `opacity`                                   | Supported                                                                                                                     |
| `filter`, `backdropFilter`                  | `<filter-function>`, `url(data:image/svg+xml,...)` <sup>1</sup>                                                               |
...
<sup>1</sup> Inline `<filter>` markup in a `data:` URI, optionally wrapped in `<svg>`. The full SVG
primitive set applies, and references mix with filter functions. Browsers parse the same markup:
prototype on an HTML page and paste it in. `url(#id)` and external URLs are parse errors. Declare
`color-interpolation-filters="sRGB"` for threshold effects (spec default: `linearRGB`). Use the
render-level `dithering` option for error-diffusion dithering.
