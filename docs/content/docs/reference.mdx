...
<sup>1</sup> Widths round to whole pixels, like browsers do. A width under `1px` becomes `1px`, so
hairlines always draw. Any other width rounds down, so `1.5px` draws as `1px`.
