...
| `content`                             | Supported                                                                                                                                                  |
| `listStyleType`                       | `none`, `disc`, `circle`, `square`, `decimal`, `decimal-leading-zero`, `lower-alpha`, `upper-alpha`, `lower-roman`, `upper-roman`, `<string>` <sup>1</sup> |
| `listStylePosition`                   | `outside`, `inside`                                                                                                                                        |
| `listStyleImage`                      | `none`, `<image>`                                                                                                                                          |

<sup>1</sup> A `display: list-item` box draws its marker as text, so `disc`, `circle` and `square`
come out as `•`, `◦` and `■` in the item's own font. `<ol start>` and `<li value>` set the count.
The `listStyle` shorthand sets all three properties at once.
