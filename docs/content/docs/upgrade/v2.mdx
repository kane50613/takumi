---
title: Upgrade to v2
description: Upgrade Takumi from v1 to v2 and migrate JavaScript APIs, Rust imports, resources, and CSS defaults.
---

v2 makes fonts and images explicit per-render resources and changes several CSS defaults. This guide covers migration from v1, including changes restored in later v2 releases. See the [release notes](https://github.com/kane50613/takumi/releases) for individual versions.
...
If you use `@takumi-rs/core` or `@takumi-rs/wasm` directly, update those packages too. Follow the JavaScript or Rust sections below, then run your type checker and compare rendered output.
...
```ts title="Top-level functions are async"
...
```ts title="The Renderer constructor takes no arguments"
...
```ts title="Pass fonts per render"
...
```ts title="Replace loadFonts with registerFont"
renderer.loadFonts([archivo, geist]); // [!code --]
...
```tsx title="Pick the fallback chain with fontFamilies"
...
```tsx title="Renamed and changed options"
const image = await renderer.render(node, {
...
```tsx title="createImageResponse is removed"
...
```tsx title="Render to vector SVG"
...
```tsx title="Bare URL strings in fonts"
...
```tsx title="@takumi-rs/image-response/wasm export is removed"
...
```tsx title="border-width and outline-width"
...
```rust title="takumi is split into focused crates"
use takumi::{ // [!code --]
...
```rust title="GlobalContext becomes a Fonts context"
...
```rust title="Image output quality is modeled per format"
...
```ts title="Image output quality is modeled per format"
...
```rust title="Renamed entry points"
let measured = measure_layout(options)?; // [!code --]
let measured = measure(options)?; // [!code ++]
```

`render_for_layout` removed its `current_color` argument in v2.0 and restored it in v2.6. Keep the v1 call shape when upgrading to v2.6 or later.
...
```rust title="Remove default-flavor type parameters"
...
```rust title="BackgroundPosition is renamed to PositionValue"
...
```rust title="Core enums are #[non_exhaustive]"
match node.kind {
...
```rust title="CSS value lists are type aliases"
...
```rust title="max-width / max-height and gap representation"
...
```rust title="font_families and lang option types are resolved"
let options = RenderOptions::builder()
...
```rust title="Stream animation frames with write_animation"
...
```rust title="Parsing HTML with takumi-html"
