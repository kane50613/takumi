...
`Length` and `ColorInput` were generic over a `const` bool so one enum could default two ways (`auto` vs `0`, `currentColor` vs `transparent`). That parameter is gone: both are plain enums. The `LengthDefaultsToZero` and `ColorDefaultsToTransparent` aliases are removed — use `Length` and `ColorInput`. `BackgroundPosition` is likewise non-generic; its `ObjectPosition` and `TransformOrigin` aliases stay. Rendering is unchanged: the zero/transparent/center initial values are now declared per field.
