...
Takumi renders COLR fonts, the format behind packs like `Twemoji-COLR`. A COLR file is much smaller than a bitmap emoji font like `Noto Color Emoji`. Register one through `fonts`. The matching emoji then draw from its glyphs, with no network fetch.
