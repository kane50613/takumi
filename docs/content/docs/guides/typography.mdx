...
Reach for COLR unless you need byte-identical output with a specific platform (Apple's `sbix` Apple Color Emoji, for example). `Twemoji-COLR` is COLR; Google's `Noto Color Emoji` ships primarily as CBDT/CBLC bitmaps, with separate COLR/COLRv1 builds derived from the same artwork. If you want a per-glyph image instead of a font, see [Emoji](/docs/guides/emoji).
