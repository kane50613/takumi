---
title: Styling
description: Style Takumi templates with inline CSS, stylesheets, Tailwind utilities, and design tokens.
icon: Palette
---

Choose how to supply styles based on your project:
...
These approaches can be combined. The sections below explain which declarations take precedence.
...
```tsx twoslash title="Style one element"
<div style={{ display: "flex", padding: 48, background: "#0f172a" }}>Hello</div>
```

## CSS

Pass a CSS string or a list of stylesheets through `css`. The engine matches it by `className` or `id`.

```tsx twoslash title="Apply a stylesheet"
...
```tsx twoslash title="Embed a style tag"
<div className="card">
  {/* [!code highlight] */}
...
```tsx twoslash title="Build CSS rule objects"
import { render } from "takumi-js";

const image = await render(<div className="card">Hello Takumi</div>, {
  width: 1200,
  height: 630,
  // [!code highlight:4]
  css: [{ layer: "theme" }, { selector: ".card", style: { background: "#ffffff", padding: 48 } }],
});
```
...
The `tw` prop runs a built-in parser with no build step. It uses the same prop name as [Satori](https://github.com/vercel/satori#tailwind-css). Check class support when migrating `next/og` templates.

```tsx twoslash title="Use Tailwind utilities"
import { render } from "takumi-js";
...
```tsx twoslash title="Build classes dynamically"
...
```tsx twoslash title="Enable Preflight"
import { render } from "takumi-js";

const image = await render(<h1 tw="text-4xl">Hello Takumi</h1>, {
  width: 1200,
  height: 630,
  // [!code highlight]
...
```css title="Define Tailwind utilities"
.card {
...
```css title="Define design tokens"
:root {
  /* [!code highlight:2] */
...
```tsx twoslash title="Apply design tokens"
import { render } from "takumi-js";
...
```css title="Override tokens by viewport"
@media (width >= 1000px) {
  :root {
