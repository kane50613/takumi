...
Declare tokens in a `:root` rule, or pass them as [the `cssVariables` option](#css-variables).
...
The variables follow the CSS cascade. A media query or a selector can override them.
