---
title: Pagination
description: Control PDF page breaks, keep related content together, and set paragraph widows and orphans with CSS.
icon: Scissors
---
...
```tsx twoslash title="Break properties"
...
```tsx twoslash title="Full-page cover"
import { render } from "takumi-pdf";

const pdf = await render(
  <article>
    {/* [!code highlight] */}
    <section style={{ height: "100vh", display: "flex", alignItems: "flex-end" }}>
      <h1>Annual report</h1>
    </section>
    <p>The report starts on page two.</p>
  </article>,
  { margin: 0 },
);
```
...
```tsx twoslash title="Full-bleed cover page"
import { render } from "takumi-pdf";

const pdf = await render(
  <article>
    {/* [!code highlight] */}
    <section style={{ page: "cover", height: "100vh", background: "#1e293b" }} />
    <p>The report starts on page two, inside the document margins.</p>
  </article>,
  {
    // [!code highlight]
    css: "@page cover { margin: 0 }",
...
```tsx twoslash title="Widows and orphans"
import { render } from "takumi-pdf";
...
```tsx twoslash title="Split decorations"
import { render } from "takumi-pdf";
...
```tsx twoslash title="Repeated table headers"
...
```tsx twoslash title="Page ranges"
declare const rows: import("react").ReactNode;
// ---cut---
import { render } from "takumi-pdf";
