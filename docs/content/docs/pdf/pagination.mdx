...
A `position: fixed` box repeats on every page, laid out against the page area rather than the flowing content:
...
`inset: 0` covers the whole page area, so the box centers per page. A `fixed` box inside a transformed or filtered ancestor belongs to that ancestor instead, and paginates with it.
