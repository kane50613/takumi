...
Invalid combinations are **TypeScript type errors**. The PDF/A-2 levels and `"4"` forbid attachments. Use `"4f"` for an attachment inside a PDF 2.0 archival file. The PDF/A-3 levels require `mimeType` and `description` on each one.
