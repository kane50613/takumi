...
the paper edge, and never below the 1cm a page starts with. A short band leaves
the page looking as it did before. Set a side to a number to size it yourself,
...
Left and right hold no band. `"auto"` there is that same 1cm.
