...
Band height is measured with the real page count, and the document re-paginates until the height settles. A counter that wraps the band at page 100 grows the margin for the whole document, so the layout never shifts between pages.
