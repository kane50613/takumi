...
Set `uncoveredText` to render anyway. `"placeholder"` draws the font's own placeholder glyph, usually an empty box; a font is free to leave it empty. `"blank"` draws nothing. Neither reflows the line: the character keeps the width the font gave it, so the page reads the same either way apart from that one mark.

Whether the character survives in the text layer depends on how it shaped. On its own it leaves no trace, because a PDF carries no Unicode for a placeholder glyph. Attached to a covered character — a combining mark on a letter the font does have — it comes back through that neighbour when the text is copied.

`"placeholder"` cannot be combined with `pdfa` or `tagged: "ua1"` / `"ua2"`. Both standards forbid the glyph it draws, so the render fails naming the characters and the standard. Use `"blank"` there instead.
