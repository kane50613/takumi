...
PNG, JPEG and WebP bytes all embed. A JPEG goes in as the JPEG you passed, so the PDF carries the compression the file already had. GIF is not supported here.

A CSS `filter` needs pixels to work on, and this build decodes only PNG. An image under a filter that came in as JPEG or WebP is skipped.
