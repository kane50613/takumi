...
Links and outline entries inside the document still target a page position. veraPDF reports rule 8.8-1 on a document with headings or internal links.

### What combines with what

Invalid combinations are TypeScript type errors. PDF/UA-1 is PDF 1.7, PDF/A-4 and PDF/UA-2 are PDF 2.0, so the two generations never mix.
