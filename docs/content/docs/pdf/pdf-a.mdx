...
Heading levels come from nesting, not from the tag name. A document that opens at `h2` gets an `H1`, and a jump from `h1` to `h4` becomes `H1` then `H2`. PDF/UA rejects a sequence that skips a level or opens below `H1`, which HTML written for looks does all the time.

Tables are missing from that list. `<table>` markup is not supported, and a table built out of flex rows carries no `Table`, `TR` or `TH` elements, so a screen reader cannot navigate it by row and column. Such a file still passes PDF/UA-1, because the validator checks the tags that are there, not the ones the content deserves.

Set `tagged: false` to skip the tree when file size matters more than accessibility.

Invalid combinations are TypeScript type errors. `pdfa: "2a"` and `pdfa: "3a"` forbid `tagged: false`. `pdfa: "4"` forbids `tagged: "ua1"`, because PDF/UA-1 is PDF 1.7-only and PDF/A-4 is PDF 2.0. The `2` levels and `"4"` forbid `attachments`; `"4f"` is the PDF 2.0 level that takes them. See [Attachments](/docs/pdf/attachments).
