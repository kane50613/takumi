...
Links and outline entries still target a page position. veraPDF reports rule 8.8-1 on a document with headings or internal links.
...
- `lang` is the document language. A-levels and PDF/UA require it. PDF/UA-2 fails the render without it.
