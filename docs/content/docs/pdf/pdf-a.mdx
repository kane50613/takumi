...
Tables built from flex rows instead of `<table>` markup get no table elements. Screen readers cannot navigate those by row or column, and the file still passes PDF/UA, because the validator only checks the tags that are there.
