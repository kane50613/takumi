...
description: Add clickable PDF links, a table of contents, heading bookmarks, and document metadata.
icon: Link
---
...
```tsx twoslash title="Link to a website"
...
```tsx twoslash title="Link within the document"
import { render } from "takumi-pdf";
...
```tsx twoslash title="Generate PDF bookmarks"
...
```tsx twoslash title="Set document metadata"
import type { ReactNode } from "react";
declare const report: ReactNode;
// ---cut---
import { render } from "takumi-pdf";
