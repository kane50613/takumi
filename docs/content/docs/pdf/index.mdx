---
title: Generate PDFs
description: Generate PDF invoices and reports from JSX, HTML, and CSS with takumi-pdf, without a headless browser.
index: true
icon: BookOpen
---
...
```npm
npm i takumi-pdf @takumi-rs/helpers
```

## Render a document

Save this as `invoice.tsx` and run `bun invoice.tsx` to write `invoice.pdf`. For Node.js, compile the JSX before running the file.

```tsx twoslash title="invoice.tsx"
...
```tsx twoslash title="Set page geometry"
...
A CSS `@page` rule sets the same geometry and wins over the options, like `preferCSSPageSize` in Chromium. `size` takes a page keyword, one or two lengths, or an orientation, and `margin` takes lengths or `auto`:

```tsx twoslash title="Page geometry from CSS"
import type { ReactNode } from "react";
declare const report: ReactNode;
// ---cut---
import { render } from "takumi-pdf";
...
A named rule such as `@page cover` styles the pages of elements with `page: cover`. See [Named pages](/docs/pdf/pagination#named-pages). The `:first`, `:left`, `:right`, and `:blank` pseudo-classes are parsed but not applied yet.
...
```tsx twoslash title="Reuse a font registry"
import type { ReactNode } from "react";
...
```ts title="Initialize in the browser"
