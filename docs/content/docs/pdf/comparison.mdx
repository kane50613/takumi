...
- takumi-pdf reuses OG-image components unchanged. Its CSS coverage is narrower than Chromium's. PDF output does not support `filter: blur()`, `drop-shadow()`, or `backdrop-filter`, and a blurred `box-shadow` is approximated with bands. Tagged output is on by default since 0.3 and adds about 10 KB to this invoice. `tagged: false` removes it.
