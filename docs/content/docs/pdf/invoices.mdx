---
title: Invoices
description: Generate paginated PDF invoices with line items, totals, page numbers, and optional embedded XML.
icon: Receipt
---
...
In this example, `invoice` is your invoice data and `InvoiceDocument` is your JSX template. For a complete implementation, see the [invoice example](https://github.com/kane50613/takumi/tree/master/example/generate-invoice).

Line items flow across pages. A footer carries the page counter. `measure` returns the footer's height so you can reserve enough bottom margin:

```tsx twoslash title="Reserve space for an invoice footer"
...
```tsx twoslash title="Keep a line item together"
...
```tsx twoslash title="Attach invoice XML"
import type { ReactNode } from "react";
...
```bash title="Validate PDF/A and PDF/UA"
...
```bash title="Validate Factur-X with Mustang"
