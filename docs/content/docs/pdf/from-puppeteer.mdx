...
description: Migrate Puppeteer PDF templates to takumi-pdf, including page setup, assets, and CSS limitations.
icon: Globe
---
...
```tsx twoslash title="After: takumi-pdf"
...
| `preferCSSPageSize`, `@page`             | `@page { size; margin }` in `css`, always preferred |
...
```tsx twoslash title="Pass fetched image bytes"
declare const html: string;
// ---cut---
...
Takumi supports a subset of browser CSS. Check these differences before replacing Puppeteer:
