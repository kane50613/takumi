...
The package is published as [`takumi-js`](https://www.npmjs.com/package/takumi-js) on npm, and it exposes the `takumi-js/node` and `takumi-js/wasm` entrypoints.

### For Edge Runtime / Browser

This is relatively rare, but if you are using a Edge runtime or browser, you can use the `takumi-js/wasm` entrypoint.
