...
`fontFamilies` is the ordered list of families to try when a glyph is missing. It defaults to the families you passed, in order. Set it to pin which family wins and what backs it up.
...
A missing glyph walks the chain until a family covers it. `googleFontSubsets` builds on this. Every subset shares one name, so a single `font-family` reaches all of them.
