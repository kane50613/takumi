...
The minimal API for animated output. Usually keep one scene and animate it with stylesheet keyframes or Tailwind's animation presets. `scenes` is an array so you can stitch several into one animation, but Takumi does not tween between them.
