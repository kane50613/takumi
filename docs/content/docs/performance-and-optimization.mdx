...
For both `takumi-js/node` and `takumi-js/wasm`, the `Renderer` is the heart of Takumi's resource management. Try to reuse the renderer instance across multiple renderings.
...
Always prefer `takumi-js/node` over `takumi-js/wasm` for utilizing multiple threads.
