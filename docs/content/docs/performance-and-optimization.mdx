...
description: Reuse Takumi renderers, tune image and glyph caches, and measure cold and repeated renders.
icon: Zap
---

## Reuse the renderer

```tsx twoslash title="Reuse the renderer"
...
```ts twoslash title="Resource cache"
import { Renderer } from "takumi-js/node";

const renderer = new Renderer({ cacheMaxBytes: 64 * 1024 * 1024 }); // [!code highlight]
```
...
```ts twoslash title="Glyph cache"
import { setGlyphCacheMaxBytes } from "takumi-js";

setGlyphCacheMaxBytes(64 * 1024 * 1024); // [!code highlight]
```
...
A larger viewport fails with `Invalid viewport dimensions`. Each pixel costs 4 bytes, so a full 64 megapixel render holds 256 MiB before encoding.
