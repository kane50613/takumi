---
title: From Satori
description: Migrate from Satori to Takumi with your existing components.
---

Takumi aims to create a universal image generation library that can be used in any environment and not just limited to JavaScript runtime.

If you are migrating from `next/og`, check out the [From Next.js ImageResponse](/docs/migration/image-response) guide instead.

<Callout type="warning">
  Takumi renders your nodes/components to rasterized images directly, we don't support rendering to
  SVG at this moment.
</Callout>

## Installation

```npm
npm i takumi-js/node takumi-js/helpers
```

<Callout>
  Make sure to mark `takumi-js/node` as `external` if you are using a bundler like Vite or Webpack
</Callout>

## Code changes

```tsx twoslash
/** @jsxImportSource react */
// ---cut-before---
// @noErrors
import satori from "satori"; // [!code --]
import { Resvg } from "@resvg/resvgtakumitakumi-js"; // [!code --]
import { fromJsx } from "takumi-js/helpers/jsx"; // [!code ++]
import { Renderer } from "takumi-js/node"; // [!code ++]

// You can use variable fonts with Takumi!
// Takumi comes with Geist and Geist Mono by default,
// if this is what you want, you can just remove fonts array.
const fonts = [
  {
    name: "Inter",
    data: () => fetch("/fonts/Inter-Regular.ttf").then((res) => res.arrayBuffer()),
    style: "normal" as const,
    weight: 400,
  },
];

// [!code --:4]
const svg = await satori(<OgImage title="Satori" />, {
  fonts,
});
const buffer = new Resvg(svg).render().asPng();

// [!code ++:3]
const renderer = new Renderer({
  fonts,
});

// [!code ++:7]
const { node, stylesheets } = await fromJsx(<OgImage title="Takumi" />);
const png = await renderer.render(node, {
  width: 1200,
  height: 630,
  format: "png", // "webp" is recommended as well.
  stylesheets,
});

function OgImage({ title }: { title: string }) {
  return (
    <div>
      <h1>Hello from {title}!</h1>
    </div>
  );
}
```

Now you have Takumi setup!

If there's any issues or unexpected behavior, please open an issue on [our GitHub repository](https://github.com/kane50613/takumi/issues).
