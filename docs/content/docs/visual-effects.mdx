...
description: Aurora meshes, liquid glass, neon CRT, chrome type, halftone, and dithering with plain CSS.
icon: Wand
---
...
![Aurora mesh with film grain](https://raw.githubusercontent.com/kane50613/takumi/master/takumi/tests/fixtures-generated/showcase_aurora_grain.webp)

## Liquid glass

A lens map driven `feDisplacementMap` inside `backdrop-filter` refracts whatever sits behind the
card; most non-Chromium browsers cannot do this. Plain frosted glass is just
...
.glass {
  ...
  border-radius: 36px;
  backdrop-filter: url("data:image/svg+xml,<filter>…</filter>") blur(1px) saturate(1.25);
  box-shadow:
...
The map is not noise. Red ramps left to right and green top to bottom, so each pixel encodes its
own displacement direction; a neutral `#808000` radial keeps the center undistorted, leaving
refraction only at the rim:
...
<svg xmlns="http://www.w3.org/2000/svg" width="560" height="320">
  <defs>
    ...
    <radialGradient id="c">
      ...
      <stop offset="0.55" stop-color="#808000" />
      <stop offset="1" stop-color="#808000" stop-opacity="0" />
    </radialGradient>
  </defs>
  <rect width="560" height="320" fill="url(#x)" />
  <rect width="560" height="320" fill="url(#y)" style="mix-blend-mode:screen" />
  <rect width="560" height="320" fill="url(#c)" />
</svg>
```

The filter loads the map (as its own `data:` URI) with `feImage` and displaces the backdrop
through it. Declare `color-interpolation-filters="sRGB"` so `#808000` stays the neutral midpoint;
resvg currently applies `scale` twice, so takumi needs `scale="9"` where a browser needs
`scale="81"`:

```xml
<filter color-interpolation-filters="sRGB" x="0" y="0" width="100%" height="100%">
  <feImage href="data:image/svg+xml,…" width="560" height="320" result="map" />
  <feDisplacementMap in="SourceGraphic" in2="map" scale="9"
    xChannelSelector="R" yChannelSelector="G" result="disp" />
  <!-- merge over the source so edges displaced out of range stay covered -->
  <feMerge>
...
![Liquid glass](https://raw.githubusercontent.com/kane50613/takumi/master/takumi/tests/fixtures-generated/showcase_liquid_glass.webp)
...
![Neon CRT](https://raw.githubusercontent.com/kane50613/takumi/master/takumi/tests/fixtures-generated/showcase_neon_terminal.webp)
...
![Chrome type](https://raw.githubusercontent.com/kane50613/takumi/master/takumi/tests/fixtures-generated/showcase_chrome_text.webp)
...
![Halftone](https://raw.githubusercontent.com/kane50613/takumi/master/takumi/tests/fixtures-generated/showcase_halftone.webp)
...
![Grid hero](https://raw.githubusercontent.com/kane50613/takumi/master/takumi/tests/fixtures-generated/showcase_grid_hero.webp)
