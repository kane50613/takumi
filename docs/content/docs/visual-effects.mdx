...
A turbulence-driven `feDisplacementMap` inside `backdrop-filter` refracts whatever sits behind the
...
.glass {
  ...
  border-radius: 36px;
  backdrop-filter: url("data:image/svg+xml,<filter>…</filter>") blur(2px) saturate(1.3);
  box-shadow:
...
<filter x="0" y="0" width="100%" height="100%">
  <feTurbulence type="turbulence" baseFrequency="0.008 0.014" numOctaves="1" seed="4" result="raw" />
  <!-- the displacement map is read premultiplied: force its alpha opaque first -->
  <feComponentTransfer in="raw" result="noise">
