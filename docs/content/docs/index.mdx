...
`takumi-js` bundles both bindings: the native renderer (`@takumi-rs/core`) and the WebAssembly one (`@takumi-rs/wasm`). It picks the right one at runtime — native on Node.js, WebAssembly on Cloudflare Workers, Vercel Edge, Deno, the browser, and Nitro. No extra install.

On a Node.js host that can't load a native addon (a WebContainer, an unsupported platform), pass a WASM binary as `module` to force the WebAssembly renderer:

```tsx twoslash
import { render } from "takumi-js";
import module from "takumi-js/wasm";

const png = await render(<div tw="w-full h-full flex">Hello Takumi</div>, {
  width: 1200,
  height: 630,
  module,
});
