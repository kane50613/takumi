...
A bare `<div>` is `block` where satori forces `display: flex`. Takumi supports Flexbox, Grid, and block layout where satori is flex-only, and rasterizes directly instead of returning an SVG to convert. See the full [comparison to satori](/docs/comparison-to-satori).
