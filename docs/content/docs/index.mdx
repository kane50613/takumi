...
Takumi renders JSX, raw HTML, and structured node trees through a full CSS layout and compositing pipeline, then outputs PNG, JPEG, WebP, ICO, GIF, APNG, or raw video frames.

This documentation covers the JavaScript bindings (`takumi-js`).
...
Takumi also emits **vector SVG** output: call `renderSvg()` to get a real `<svg>` document (`<rect>`, `<path>`, gradients, glyph outlines, embedded images) instead of a raster bitmap — the same layout, as a scalable vector.
