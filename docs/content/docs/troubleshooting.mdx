...
description: Fix Takumi import, bundler, font, image, and layout errors using their messages and runtime-specific solutions.
icon: Bug
---

Search for the error message from your build or server logs. Start with the import table if the failure depends on your runtime.

## Pick the right entry

Check that your import matches the runtime and bundler.
...
Use the runtime-specific entries below when automatic resolution selects the wrong build.
...
```ts title="Export default doesn't exist in target module"
...
current platform is missing. Check package hoisting and the deployment target below.
...
```ts title="No registered font covers क (U+0915)"
...
The PDF renderer rejects uncovered characters because they would be missing from both the visible page and its text layer.
...
Shrink the viewport. 8192 × 8192 is the largest square that fits.

A zero-sized viewport raises the same error. To go larger, render in tiles and stitch them. See [Render large canvases](/docs/performance-and-optimization#render-large-canvases).
...
```tsx title="The layout is not what I expected"
