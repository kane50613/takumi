...
Add a code block `title` when it identifies the file to edit or clarifies the task. Omit it for self-explanatory commands such as `npm install`, or when it only repeats the nearby heading. Use `twoslash` for complete TypeScript examples and `[!code highlight]` to draw attention to the relevant lines. Keep migration diffs on `[!code ++]` and `[!code --]`. Define sample inputs or explain where they come from. GitHub READMEs should use plain fenced code and nearby file labels because GitHub does not render these annotations.
