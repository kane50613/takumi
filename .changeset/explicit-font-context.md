...
- Remove the imperative `loadFont`/`loadFontSync` and `loadFonts`. Register fonts via `registerFonts` (returns the resolved families per font) or the per-render `fonts` option.
