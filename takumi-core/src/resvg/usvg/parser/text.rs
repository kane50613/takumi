...
fn collect_text_chunks_impl(
  ...
  for child in parent.children() {
    ...
    let font_optical_sizing = match parent.find_attribute::<&str>(AId::FontOpticalSizing) {
      Some("none") => crate::resvg::usvg::FontOpticalSizing::None,
      _ => crate::resvg::usvg::FontOpticalSizing::Auto, // "auto" or missing (browser default)
    };

    let mut text_length = parent.try_convert_length(AId::TextLength, Units::UserSpaceOnUse, state);
  ...
  for n in nodes {
    if let Some(len) = n.try_attribute::<Length>(AId::BaselineShift) {
      if len.unit == LengthUnit::Percent {
        ...
        let n =
          super::units::convert_length(len, n, AId::BaselineShift, Units::ObjectBoundingBox, state);
        shift.push(BaselineShift::Number(n));
      }
