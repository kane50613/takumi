...
fn collect_children(
  ...
  for child in filter.children() {
    ...
    let filter_subregion =
      match resolve_primitive_region(child, tag_name, units, state, object_bbox, filter_region) {
        Some(v) => v,
        None => break,
      };
    ...
    let kind = match tag_name {
      ...
      EId::FeConvolveMatrix => {
        convert_convolve_matrix(child, &primitives).unwrap_or_else(create_dummy_primitive)
      }
  ...
  match fe.attribute(AId::Type) {
    Some("saturate") => {
      if let Some(list) = fe.attribute::<Vec<f32>>(AId::Values) {
        if !list.is_empty() {
          let n = crate::resvg::usvg::f32_bound(0.0, list[0], 1.0);
...
fn convert_component_transfer(fe: SvgNode, primitives: &[Primitive]) -> Kind {
  ...
  for child in fe.children().filter(super::svgtree::SvgNode::is_element) {
...
fn convert_std_dev_attr(fe: SvgNode, scale: Size, default: &str) -> (PositiveF32, PositiveF32) {
  ...
  let n1 = parser.next().and_then(std::result::Result::ok);
  let n2 = parser.next().and_then(std::result::Result::ok);
  // `stdDeviation` must have no more than two values.
  // Otherwise we should fallback to `0 0`.
  let n3 = parser.next().and_then(std::result::Result::ok);
...
fn convert_specular_lighting(
  ...
  let specular_exponent = crate::resvg::usvg::f32_bound(1.0, specular_exponent, 128.0);
  ...
  match child.tag_name() {
    ...
    Some(EId::FeSpotLight) => {
      ...
      let specular_exponent =
        PositiveF32::new(specular_exponent).unwrap_or_else(|| PositiveF32::new(1.0).unwrap());
