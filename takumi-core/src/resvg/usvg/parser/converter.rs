...
impl<'a, 'input: 'a> SvgNode<'a, 'input> {
  ...
  pub fn is_visible_element(&self, opt: &crate::resvg::usvg::Options) -> bool {
...
fn convert_path(
  ...
  if let Some(fill) = fill.as_mut() {
    if let Some(ContextElement::PathNode(context_transform, context_bbox)) = fill.context_element {
      process_paint(
        &mut fill.paint,
        true,
        context_transform,
  ...
  if let Some(stroke) = stroke.as_mut() {
    if let Some(ContextElement::PathNode(context_transform, context_bbox)) = stroke.context_element
    {
      process_paint(
        &mut stroke.paint,
        true,
