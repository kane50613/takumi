...
const raw = { width: 10, height: 10, format: "raw" } as const;
...
test("stylesheets warns once", async () => {
  ...
  await render(node, { ...raw, stylesheets: [sheet] });
  await render(node, { ...raw, stylesheets: [sheet] });
...
test("css string renders like the stylesheets list", async () => {
  const fromCss = await render(node, { ...raw, css: sheet });
  const fromList = await render(node, { ...raw, stylesheets: [sheet] });
