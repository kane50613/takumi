...
struct Walk<'c> {
  ...
  /// Whether form controls become fillable fields, which puts each widget
...
fn role(node: &RenderNode, walk: &mut Walk, nesting: Nesting) -> Option<TagKind> {
  ...
  match tag_name {
    ...
    _ if walk.forms && is_form_control(source) => Some(Tag::Form.into()),
...
/// Every piece of text under `node`, as written.
pub(crate) fn raw_text(node: &RenderNode) -> String {
