...
The `square` counter style now generates `▪` (U+25AA BLACK SMALL SQUARE), matching the CSS Counter Styles definition browsers use, instead of the oversized `■`. Bullet markers (`disc`, `circle`, `square`) separate from the item with Blink's fixed 7px gap instead of the suffix space, so they no longer hug the text.
